...
class _UsuariosScreenState extends State<UsuariosScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (index == 0) {
        res = await _repo.pagedActivos(page: state.page, pageSize: state.pageSize, q: q, sort: state.sort, order: state.asc ? 'asc' : 'desc');
      } else if (index == 1) {
        res = await _repo.pagedInactivos(page: state.page, pageSize: state.pageSize, q: q, sort: state.sort, order: state.asc ? 'asc' : 'desc');
      } else {
        res = await _repo.pagedTodos(page: state.page, pageSize: state.pageSize, q: q, sort: state.sort, order: state.asc ? 'asc' : 'desc');
      }
  ...
  Future<void> _toggleUserStatus(Usuario u) async {
    ...
    try {
      ...
      // No recargamos la pestaña actual (_loadData) porque ya quitamos el item visualmente arriba (paso 2)
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: Text('Crear Nuevo Usuario', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: cs.onSurface)),
                    ),
                        ...
                        child: Column(
                          children: [
                            TextFormField(
                              ...
                              decoration: _modernInputDeco('Nombre completo', Icons.person_outline),
                            ...
                            TextFormField(
                              ...
                              decoration: _modernInputDeco('Correo electrónico', Icons.alternate_email),
                            ...
                            TextFormField(
                              ...
                              decoration: _modernInputDeco('Contraseña', Icons.lock_outline).copyWith(
                                suffixIcon: IconButton(icon: Icon(obscurePass ? Icons.visibility_off : Icons.visibility, color: cs.primary), onPressed: () => setInnerState(()=>obscurePass=!obscurePass))
                              ),
                            ...
                            DropdownButtonFormField<int>(
                              ...
                              items: UserRole.values.map((r) => DropdownMenuItem(value: r.id, child: Text(r.label))).toList(),
                            ...
                            SizedBox(
                              ...
                              child: FilledButton(
                                ...
                                onPressed: isSaving ? null : () async {
                                  ...
                                  try {
                                    await _repo.create(nombre: nombreCtrl.text.trim(), correo: correoCtrl.text.trim(), password: passCtrl.text, idRol: idRol);
                                    if (mounted) {
                                      Navigator.pop(innerCtx);
                                      _loadData();
                                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Usuario creado'), behavior: SnackBarBehavior.floating));
                                    }
                                  } catch (e) {
                                    setInnerState(() => isSaving = false);
                                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.toString()), backgroundColor: cs.error));
                                  }
                                },
                                child: isSaving ? SizedBox(width: 24, height: 24, child: CircularProgressIndicator(color: cs.onPrimary, strokeWidth: 2)) : const Text('Registrar Usuario', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                              ),
                            ),
                          ],
                        ),
                  ...
                  children: [
                    Padding(
                      ...
                      child: Row(
                        children: [
                          const Spacer(),
                          IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(innerCtx)),
                        ],
                      ),
                    ),
                    ...
                    Hero(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: cs.primary.withOpacity(0.5), width: 2)),
                        child: CircleAvatar(
                          radius: 40,
                          backgroundColor: cs.primaryContainer,
                    ...
                    const SizedBox(height: 16),
                    Text('Editar Perfil', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: cs.onSurface)),
                    Text('Actualiza la información del usuario', style: TextStyle(fontSize: 14, color: cs.onSurfaceVariant)),
                      ...
                      child: Form(
                        ...
                        child: Column(
                          children: [
                            ...
                            DropdownButtonFormField<int>(
                              value: idRol,
                              decoration: _modernInputDeco('Rol asignado', Icons.badge_outlined),
                              items: UserRole.values.map((r) => DropdownMenuItem(value: r.id, child: Text(r.label))).toList(),
                              onChanged: (v) => setInnerState(() => idRol = v!),
                              icon: const Icon(Icons.keyboard_arrow_down_rounded),
                            ),
                          ...
                          Expanded(
                            child: FilledButton(
                              onPressed: isSaving ? null : () async {
                                ...
                                setInnerState(() => isSaving = true);
                                try {
                                  await _repo.update(idUsuario: u.id, nombre: nombreCtrl.text.trim(), correo: correoCtrl.text.trim(), idRol: idRol);
                                  if (mounted) {
                                    Navigator.pop(innerCtx);
                                    _loadData();
                                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Guardado'), behavior: SnackBarBehavior.floating));
                                  }
                                } catch (e) {
                                  setInnerState(() => isSaving = false);
                                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.toString()), backgroundColor: cs.error));
                              ...
                              child: isSaving ? SizedBox(width: 20, height: 20, child: CircularProgressIndicator(color: cs.onPrimary, strokeWidth: 2)) : const Text('Guardar'),
                            ),
                          ),
                        ],
                      ),
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: const [
            Tab(text: 'Activos', icon: Icon(Icons.check_circle_outline, size: 20)),
            Tab(text: 'Inactivos', icon: Icon(Icons.block_outlined, size: 20)),
            Tab(text: 'Todos', icon: Icon(Icons.list_alt_rounded, size: 20)),
          ],
        ),
      ),
      ...
      body: Column(
        children: [
          ...
          if (_error != null)
            Container(
              color: cs.errorContainer, padding: const EdgeInsets.all(8), width: double.infinity,
                  ...
                  Positioned(
                    ...
                    child: Center(
                      child: Container(
                        ...
                        decoration: BoxDecoration(color: cs.inverseSurface.withOpacity(0.9), borderRadius: BorderRadius.circular(20)),
                        child: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            SizedBox(width: 14, height: 14, child: CircularProgressIndicator(strokeWidth: 2, color: cs.onInverseSurface)),
                            const SizedBox(width: 10),
                            Text('Cargando...', style: TextStyle(color: cs.onInverseSurface, fontSize: 12)),
                          ],
                        ),
                      ),
                    ),
          ...
          Expanded(
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(30), borderSide: BorderSide(color: cs.primary, width: 1)),
  ...
  Widget _buildPaginator(_TabState state) {
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          Text('$from-$to de $total usuarios', style: TextStyle(color: Theme.of(context).colorScheme.secondary, fontWeight: FontWeight.w500)),
          Row(
            children: [
              IconButton(
                icon: const Icon(Icons.chevron_left),
                onPressed: state.page > 1 ? () { setState(() => state.page--); _loadData(); } : null,
              ),
              IconButton(
                icon: const Icon(Icons.chevron_right),
                onPressed: to < total ? () { setState(() => state.page++); _loadData(); } : null,
              ),
            ],
          )
        ],
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.person_search_outlined, size: 80, color: Theme.of(context).disabledColor.withOpacity(0.3)),
          const SizedBox(height: 16),
          const Text('No se encontraron usuarios', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w500)),
        ],
      ),
    );
  }
      ...
      child: Card(
        ...
        child: SingleChildScrollView(
          ...
          child: DataTable(
            ...
            dataRowMaxHeight: 52,
            columns: [
              _col('Cédula', 'cedula', state), // <--- RESTAURADO
            ...
            rows: users.asMap().entries.map((entry) {
              ...
              final isEven = i % 2 == 0;
              return DataRow(
                color: MaterialStateProperty.all(isEven ? Colors.transparent : cs.surfaceContainerHighest.withOpacity(0.2)),
                cells: [
                  DataCell(Text(u.cedula ?? '—', style: const TextStyle(fontFamily: 'monospace', fontWeight: FontWeight.w600))), // <--- RESTAURADO
                  DataCell(Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      CircleAvatar(
                  ...
                  DataCell(Text(u.creadoEn.toString().split(' ')[0], style: const TextStyle(fontFamily: 'monospace'))),
                  DataCell(Row(
                    children: [
                      IconButton(icon: const Icon(Icons.edit_outlined, size: 20), onPressed: () => _openEditUserDialog(u)),
                      IconButton(
                        icon: Icon(_tab.index == 1 ? Icons.check_circle_outline : Icons.block_outlined, size: 20), 
                ...
                children: [
                  Hero(
                    ...
                    child: CircleAvatar(
                      ...
                      child: u.avatarUrl == null ? Icon(Icons.person, color: Theme.of(context).colorScheme.onPrimaryContainer) : null,
                    ),
                  ),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Row(
                          children: [
                            Flexible(child: Text(u.nombre, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis)),
                            const SizedBox(width: 8),
                            _RolePill(rol: u.rol, compact: true),
                          ],
                        ),
                        // CÉDULA RESTAURADA AQUÍ
                        if (u.cedula != null)
                          Padding(
                            ...
                            child: Row(
                              children: [
                                ...
                                Text(u.cedula!, style: TextStyle(color: Theme.of(context).hintColor, fontSize: 13, fontWeight: FontWeight.w600)),
                              ],
                            ),
                          ),
                        const SizedBox(height: 2),
                        Text(u.correo, style: TextStyle(color: Theme.of(context).hintColor, fontSize: 13)),
                      ],
                    ),
                  ),
                  Column(
                    children: [
                      ...
                      IconButton(
                        icon: Icon(_tab.index == 1 ? Icons.check_circle : Icons.remove_circle_outline),
                        iconSize: 20,
                        color: _tab.index == 1 ? Colors.green : Colors.red.withOpacity(0.7),
                        onPressed: () => _toggleUserStatus(u),
                      ),
                    ],
                  )
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Text(
        ...
        style: TextStyle(color: fg, fontSize: compact ? 10 : 11, fontWeight: FontWeight.bold),
      ),
    );
  }
}