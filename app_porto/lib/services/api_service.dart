...
class ApiService {
  ...
  static Future<Map<String, dynamic>> updateProfile({
    ...
    required String nombre,
    String? avatarUrl,
    String? avatarPublicId, // opcional si lo manejas en el back
  }) async {
    final url = Uri.parse('$API_BASE/me');
  ...
  static Future<String> uploadAvatar({
    ...
    MediaType? _typeFor(String name) {
      ...
      switch (ext) {
        ...
        default:
          return null; // si no reconocemos, lo dejamos null
      }
    }
    ...
    req.files.add(
      http.MultipartFile.fromBytes(
        ...
        bytes,
        filename: filename,
        contentType: _typeFor(filename), // 👈 MIME correcto
      ),
    );
  ...
  // =========================================================
  //                    USUARIOS (solo ADMIN)
  //  (Compatibilidad: métodos simples SIN paginación)
  // =========================================================
  ...
  /// PUT /usuarios/:id  (admin) — editar (y opcionalmente activar/desactivar)
  static Future<Map<String, dynamic>> putUsuario({
    ...
    required String correo,
    required int idRol,
    bool? activo,               // <-- opcional, útil para activar/desactivar desde PUT
    String? avatarUrl,          // opcional
    bool? verificado,           // opcional
    int? idAcademia,            // opcional
  }) async {
  ...
  /// DELETE /usuarios/:id  (admin) -> desactiva usuario (soft delete)
  ...
  /// POST /usuarios/:id/activar  (admin) -> reactiva usuario
